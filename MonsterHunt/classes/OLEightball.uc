...
var bool bFireLoad, bTightWad, bInstantRocket, bAlwaysInstant, bClientDone, bPendingLock;
...
function float RateSelf( out int bUseAltMode )
{
	...
	if ( (P.Base != None) && (P.Base.Velocity != vect(0,0,0)) && !P.CheckFutureSight(0.1) )
		return 0.1;
...
simulated function PlayRFiring()
{
	if ( Owner.IsA('PlayerPawn') )
	{
		...
		PlayerPawn(Owner).ClientInstantFlash( -0.40, vect(650,450,190));
	}
	if ( Affector != None )
		Affector.FireEffect();
	if ( !bFireLoad )
		PlayOwnedSound( AltFireSound, SLOT_None, 4.0 * Pawn(Owner).SoundDampening);
	PlayAnim('Fire',0.62,0.05);
}
...
state AltFiring
{
	function Tick( float DeltaTime )
	{
		...
		if ( P == None )
			return;
		if( (P.bAltFire==0) || (RocketsLoaded > 5) )  // If if Fire button down, load up another
 			GoToState('FireRockets');
	}
...
ignores Fire, AltFire;
	...
	function AnimEnd()
	{
		if ( (AmmoType.AmmoAmount > 0) && (AnimSequence == 'Fire') ) 
		{	
			PlayRotating();
			RocketsLoaded = 1;
			return;
...
defaultproperties
{
    ...
    ItemName="Eightball"
    PlayerViewOffset=(X=1.90,Y=0.00,Z=-1.89),
